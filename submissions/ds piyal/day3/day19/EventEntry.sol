...
contract EventEntry {
    ...
    uint256 public maxAttendeeCount;
    ...
    modifier onlyOrganizer() {
        require(msg.sender == organizer, "Only the organizer can call");
        _;
    }
    ...
    function checkIn(bytes memory _signature) external {
        require(isEventActive, "This event is not active");
        require(block.timestamp >= eventDate, "Event has not started yet");
        require(block.timestamp <= eventDate + 1 days, "Event has ended");
        require(!hasAttended[msg.sender], "Already checked in");
        require(attendeeCount < maxAttendeeCount, "Max attendees reached");
