...
contract PreOrderToken is MyToken {
    ...
    event SaleFinalized(uint256 totalRaised, uint256 totalRokensSold);
    ...
    constructor(
        ...
        saleEndTime = block.timestamp + saleDurationInSencods;
    ...
    function transfer(address _to,uint256 _value)public override returns(bool){
        if(!finalized && msg.sender !=address (this)&& initialTransferDone){
            require(false,"Takens are locked until sale is finalized");
        }
    ...
    if(!finalized && _from !=address(this)){
        require(false,"Tokens are lockef until sale is finalized");
    }
        return super.transfer(_to,_value);
    }

    function finalizeSale() public payable {
        require(msg.sender == _projectOwner,"Only owner can call this function");
        require(!finalized,"Sale is already finalized");
        require(block.timestamp > saleEndTime,"Sale not finished yet");
        finalized = true;
        uint356 tokensSold = totalSupply - balanceOf[address(this)];
