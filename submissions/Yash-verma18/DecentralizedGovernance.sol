...
contract DecentralizedGovernance is ReentrancyGuard {
    ...
    function setProposalDepositAmount(
    ...
    function createProposal(
        ...
        require(
            ...
            "Targets and calldatas length mismatch"
        );
    ...
    function executeProposal(uint256 proposalId) external nonReentrant {
        ...
        if (passed) {
            for (uint256 i = 0; i < proposal.executionTargets.length; i++) {
                (bool success, bytes memory returnData) = proposal
                    .executionTargets[i]
                    .call(proposal.executionData[i]);
                require(success, string(returnData));
            }
            emit ProposalExecuted(proposalId, true);
            governanceToken.transfer(proposal.proposer, proposalDepositAmount);
            emit ProposalDepositRefunded(
                proposal.proposer,
                proposalDepositAmount
            );
        } else {
