...
contract MiniDexPair is ReentrancyGuard{
    ...
    constructor(address _tokenA, address _tokenB){
        ...
        require(_tokenA != address(0) && _tokenB != address(0), "0 address");
        tokenA = _tokenA;
        tokenB = _tokenB;
    }
    ...
    function addLiquidity(uint256 amountA, uint256 amountB) external nonReentrant{
        ...
        if(totalLPSupply == 0){
            ...
            lpToMint = min(
                (amountA * totalLPSupply)/reserveA, (amountB * totalLPSupply)/reserveB
            );
        }
        require(lpToMint > 0, "0 lp to be minted");
    ...
    function removeLiquidity(uint256 lpAmount) external nonReentrant{
        require(lpAmount > 0 && lpAmount <= lpBalances[msg.sender], "Invalid amount");
